...
태스크의 Description·Acceptance Criteria·의존을 읽고, 관련 파일(정의·참조·테스트·설정)을 먼저 읽는다. 태스크에 명시되지 않아 판단이 필요한 사항(설계 방향, 라이브러리·API 선택, 범위 경계, 네이밍, 에러 처리 정책 등)은 구현 전에 `AskUserQuestion` 으로 모두 해소한다. 추측으로 진행하지 않고, 답변에서 파생된 의문도 다시 묻는다. 이미 태스크에 명시됐거나 판단 요소가 없으면 묻지 않고 바로 구현한다. 착수 직전 상태를 In Progress 로 바꾼다:
...
1. **준비**: 범위·완료 조건 정의, 관련 파일 읽기. 판단이 필요한 사항은 `AskUserQuestion` 으로 사전 해소. 착수 직전 상태를 `[→]` 로 갱신.
...
**세션 위생**: PLAN 은 세션 리셋 후에도 재개 가능한 진실원본이다. 커밋 직후 경계에서 `/clear` 후 재진입해도 손실이 없다.
