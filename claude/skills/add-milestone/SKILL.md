...
말고 `AskUserQuestion` 으로 묻는다:

- **단일 태스크로** — `Skill("add-task")` 로 위임하고 이 스킬은 종료한다.
...
2. 마일스톤의 목표·범위와 **태스크 분해안**(각 태스크의 완료 조건·의존·우선순위·단독실행)을
   정리하고, 애매한 점이 남으면 모두 해소될 때까지 `AskUserQuestion` 으로 인터뷰한다. 분해안
...
대상 플랜 파일과 다음 `T-N`·`M-N` 번호는 `bash ~/.claude/skills/add-milestone/plan-context.sh`
**1회 호출**로 확인한다(`primary` 가 대상 파일). Glob/Grep 으로 따로 찾지 않는다.
