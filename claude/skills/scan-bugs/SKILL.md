...
`README.md`/`AGENTS.md`/`CLAUDE.md` 가 있으면 먼저 읽어 도메인·아키텍처를 파악한다. **직접 읽기가 기본이다.** 핵심 코드 약 5,000 라인 이하면 핵심 파일 전부를 `Read` 로 정독한다. 그 이상이어도 먼저 진입점·인증·라우팅·핵심 도메인 모듈을 직접 정독하고, **그러고도 컨텍스트에 안 들어가는 나머지에 한해** `Agent(Explore)` 요약으로 대체한다 — 요약은 원문보다 놓치는 것이 많으니 넘칠 때의 폴백이지 기본 경로가 아니다. 약 10,000 라인을 넘으면 사용자에게 알리고 핵심 영역부터 진행한다. 정독 중 의심스러운 패턴은 메모해 뒀다가 서브에이전트 프롬프트의 "특별히 봐달라" 항목으로 전달한다.
...
**거절되면 분류기를 의심한다.** cyber/bio 안전 분류기가 정상적인 보안 리뷰를 오탐으로 거절할 수 있다. 프롬프트를 바꿔 가며 우회하지 말고 무엇이 거절됐는지 사용자에게 그대로 보고한다 — 거절을 "이슈 없음" 으로 넘기면 거짓 음성이 된다. `--fallback-model` 은 `--print` 전용이라 대화형 세션에는 안 걸린다. 같은 규칙이 `scan-bugs`·`scopic-deep-review`·`resolve-security-alerts` 세 곳에 있다 — repo 가 갈려 공유 파일을 못 쓰니 한쪽을 고치면 나머지도 함께 고친다.
