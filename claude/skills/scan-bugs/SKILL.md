...
**거절되면 분류기를 의심한다.** cyber/bio 안전 분류기가 정상적인 보안 리뷰를 오탐으로 거절할 수 있다. 프롬프트를 바꿔 가며 우회하지 말고 무엇이 거절됐는지 사용자에게 그대로 보고한다 — 거절을 "이슈 없음" 으로 넘기면 거짓 음성이 된다. `--fallback-model` 은 `--print` 전용이라 대화형 세션에는 안 걸린다.
