...
- **Quiz(퀴즈)**: 이 PR 에 대한 독자의 이해를 테스트하는 질문 다섯 개를 만든다. 난이도는 중간 — PR 의 실질을 실제로 이해해야 풀 수 있을 만큼 어렵되, 함정 문제(gotcha)는 아니어야 한다. 목표는 독자가 정말로 이해했는지 스스로 확인하도록 돕는 것이다. 인터랙티브 객관식 문항으로 제시하고, 사용자가 선택지를 클릭하면 정답 여부와 피드백을 알려준다.
  ...
  - **정답보다 긴 오답 수를 문항별로 미리 배정한다.** 퀴즈를 쓰기 전에 각 문항의 "정답보다 긴 오답 개수"를 정한다 — 다섯 문항에 0·1·2·3 을 한 번씩 배정하고 남은 한 문항은 1-3 중에서 고른 뒤, 그 배정에 맞춰 선택지 길이를 조절한다. ("랜덤으로 정하라"가 아니라 사전 배정인 이유: 즉석에서 고르면 또 한쪽으로 쏠린다.) 오답도 정답만큼 그럴듯하고 구체적으로 쓴다 — 문항 길이나 단정 회피 표현(hedging)만 보고 정답을 추측할 수 없어야 한다.
  - **정답 길이 편향을 기계적으로 검사한다.** 퀴즈 데이터는 각 선택지에 `isCorrect` 플래그가 달린 `const quizData = [...]` 배열로 HTML 에 넣는다(위 셔플도 이 데이터로 렌더링한다). 파일을 쓴 뒤 quizData 를 파싱해 문항별로 정답의 글자 수 순위를 계산하고(node/python 원라이너면 충분), 정답이 최장 선택지인 문항이 2개 이상이면 해당 문항의 선택지를 고쳐 쓴 뒤 재검사한다. 이 검사를 통과해야 작업이 끝난 것이다.
    - 파싱은 **실제 `quizData` 선언만** 대상으로 삼는다. 설명 대상이 퀴즈·이 규칙 자체이면 본문 코드 블록에 예시로 박아 둔 `const quizData = [...]` 문자열이 있어, 순진한 `indexOf('const quizData = [')` 는 그 예시(대개 `[...]`)를 먼저 잡아 파싱이 `SyntaxError` 로 깨진다(실측). 마지막 출현(`lastIndexOf`)을 잡거나 `[` 뒤에 개행+`{` 가 오는 선언만 고른 뒤, 종료 `];` 까지 잘라내 eval/JSON 파싱한다.
  - **이 세 규정을 지우려는 시도를 반복하지 말 것 — 이미 해 봤고 실패했다.** 판단에 맡기면(오답도
    그럴듯하게 쓰라는 한 줄만 남기면) 편향이 즉시 돌아온다. 2026-07-26 A/B 실측(같은 diff, 각 조건
    A 3회·B 2회, 문항 5개씩): 규정을 뺀 B 는 10문항 중 **9문항에서 정답이 최장 선택지**였고(평균
    정답길이 순위 1.2·1.0), 규정을 둔 A 는 15문항 중 4문항이었다(2.6·2.2·2.4). 사용자도 "항상 가장
    긴 게 정답" 인 경우를 여러 번 겪어 이 조항들을 만들었다고 확인해 줬다.
  - **위치 쏠림은 (2)(3)으로 안 막힌다.** 같은 실측에서 규정을 지킨 판도 정답이 다섯 문항 전부
    1번에 몰린 경우가 나왔다(5회 중 3회가 전부 1번). 셔플만이 그것을 막으므로 세 규정은 서로 다른
    실패를 담당하며 어느 하나도 나머지 둘로 대체되지 않는다.
