...
- Invoke this skill explicitly. Do not activate it implicitly for ordinary frontend work.
- Use only the harness-native image generation tool. Never use `scripts/generate-image.mjs`, an OpenAI API key, or another metered image API as a fallback.
- If native image generation is unavailable or its included quota is exhausted, stop image generation and report it instead of switching billing paths.
