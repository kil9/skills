...
6. Use only the harness's native image tool when generation or editing is needed. If it is unavailable or its included quota is exhausted, report the block; never use `generate-image.mjs` or another API fallback.
