 ...
 * unavailable. The local policy permanently disables the optional anonymous
 * choice ping; `--chosen` is retained only as a no-op compatibility path.
 *
 * Env vars:
 ...
 *   IMPECCABLE_NO_TELEMETRY — retained for upstream compatibility; telemetry is always disabled here.
 */
...
function telemetryDisabled() {
  // Local policy: never transmit the user's chosen direction.
  return true;
}
...
rivals to your habitual layout, and keep only what makes this product clearer.${grainNote}\n`
  ...
  these fresh challengers.\n`
    : '';
  // Local policy disables choice telemetry, so never instruct the harness to
  // make the otherwise optional --chosen follow-up request.
