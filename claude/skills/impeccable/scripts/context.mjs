...
// ─── Update check ──────────────────────────────────────────────────────────
// Upstream keeps this plumbing for compatibility, but local policy pins the
// audited version and fetchLatestSkillVersion() never performs a network poll.
...
async function fetchLatestSkillVersion() {
  // Local policy pins updates in the managed skills repository. Do not poll
  // upstream or invite an in-session npx update that would bypass the audit.
  return null;
}
...
// Local policy: image generation must stay on the harness-native path so it
// uses included subscription quota and can never fall through to API billing.
function appendImageGenDirective(parts) {
  parts.push([
    'IMAGE_GEN_POLICY: Use only the harness-native image generation tool.',
    'Never invoke generate-image.mjs, OPENAI_API_KEY, or another metered API fallback.',
    'If native generation is unavailable or its included quota is exhausted, report that and stop image generation.',
  ].join(' '));
}
