#!/bin/bash
# migrate-to-backlog 전용 헬퍼: 이관 대상 플랜 파일 결정·계열별 번호 파악·전문 덤프를 한 번에.
# (레거시 PLAN.md 를 읽는 스킬은 이제 migrate-to-backlog 뿐이다. 플랜 백엔드로 새로 작업하는
#  스킬군은 전부 삭제됐고 backlog 가 유일한 백엔드다.)
# 사용법: plan-context.sh [--dump]
#   기본   : primary 플랜 파일 + 파일별 T/I/M 현재 최대 번호와 다음 번호
#   --dump : 각 플랜 파일 전문을 헤더와 함께 출력 (이관 전 전체 파악용)
# 플랜 파일이 하나도 없으면 exit 1 — 호출측(스킬)이 init-backlog 을 안내한다.
