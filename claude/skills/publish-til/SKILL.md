...
- 인자가 없거나 슬러그 형식이 아니면(주제 설명문 등) **콘텐츠 주제를 파악해 kebab-case 슬러그를 스스로 정한다** — 후보를 물어보지 않는다. 인자가 주제 설명문이면 그 문장 자체를 콘텐츠 요구사항(글감)으로 삼는다. `<연도>/` 가 시간을 분리하므로 연도 접두사는 붙이지 않는다.
- 충돌 검사는 그대로다. §0 프리플라이트에 슬러그를 넘겼으면 끝났고, 스스로 정한 슬러그는 `til-preflight.sh <slug>` 재실행으로 확인한다. **exit 3(이미 존재)일 때만** 사용자에게 묻는다 — 다른 슬러그로 갈지, 덮어쓸지. 덮어쓰기는 명시적으로 원할 때만 진행한다.
...
한 줄 제목은 **스스로 정한다** — 묻지 않는다. 루트 갤러리에 이미 있는 제목들의 결(길이·어투)을 따르고, 슬러그를 케이스만 바꾼 것보다 내용을 드러내는 쪽을 고른다. 정한 제목은 §7 에서 알린다.
...
- 신규 디렉터리 경로와 커밋 SHA
- **스스로 정한 슬러그와 한 줄 제목**(사용자가 고르지 않았으므로 무엇으로 박혔는지 명시한다)
