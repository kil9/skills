...
- 위 수치는 opus/sonnet 팀원 실측이라 보수적이다. 팀원이 상위 모델(Fable 등)로 돌 때는 cold-start 페널티가 낮으므로 문턱을 한 단계 낮춰 병렬을 더 적극 채택해도 된다.
...
**팀원 모델**: 호출 시 지정이 없으면 `model: "opus"` 를 기본으로 명시한다(가벼운 태스크는 sonnet). 리드가 상위 모델(Fable 등)로 돌아도 팀원에게 상속시키지 않기 위함. 특별히 무거운 태스크만 리드 재량으로 그 태스크에 한해 올린다.
