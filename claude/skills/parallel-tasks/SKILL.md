---
description: 상호 의존이 없는 태스크를 에이전트 팀(팀원)으로 worktree에서 구현·커밋하고, 검증을 통과한 브랜치를 순차 ff-merge해 main에 직접 반영한다. backlog/ 가 있으면 backlog CLI 백엔드, 없으면 레거시 PLAN.md 로 동작한다. 병렬 이득이 작으면(독립·substantial 태스크 부족) 팀을 띄우지 않고 솔로로 폴백한다. 사용자가 명시적으로 herdr pane 을 지시한 경우에만(opt-in) 팀 대신 herdr pane 워커로 돌린다.
...
- **opus 4.8/sonnet 팀원**(전사 분석): 3\~5명을 띄워도 실효 동시성 약 1.8x(최악 0.73x, 직렬보다 느림), 토큰은 단일 에이전트 대비 3\~10배.
- **Opus 5 팀원 3명**(2026-07-25, task-168): 실효 동시성 **1.36x**(팀 구간 19.2분 / 팀원 wall 합 26.2분). 팀원 토큰 합 35.8M(캐시 read 포함)·캐시 read 제외 2.3M, 같은 구간 리드는 6.1M·0.17M. 단일 에이전트 대비 배수는 대조군을 안 돌려 재지 못했다.

세대가 올라가도 **cold-start 는 줄지 않았다** — 프리픽스 재구성은 모델 능력이 아니라 구조적 비용이라, 팀원별 캐시 write 가 0.18\~1.6M 인 동안 리드는 88k 였다. 그리고 실효 동시성을 실제로 좌우한 것은 모델이 아니라 **태스크 크기 균질성**이었다: 19.2분짜리 하나에 3.7·3.3분짜리 둘이라, 짧은 둘이 끝난 뒤 구간의 3/4 을 한 명이 혼자 돌았다. 따라서:

- **substantial 독립 태스크 ≥ 2개**(팀원 cold-start 를 상쇄할 만큼 큰 것. 다파일·신규 구현·리팩터급)일 때만 팀을 띄운다. 사소한 태스크(단일 파일·기계적 수정)는 팀원에 끼우지 말고 리드가 인라인 처리한다.
...
최종 보고 전에 Done 태스크가 7개 이상 쌓여 있으면 리드가 `Skill("cleanup-tasks")` 를 실행해 정리한다(완료 태스크를 completed 폴더로 이동). 팀원에게 시키지 않는다 — 모든 머지가 끝난 뒤 리드에서 한 번만 돈다.
