...
- **opus 4.8/sonnet 워커**(전사 분석): 3\~5명을 띄워도 실효 동시성 약 1.8x(최악 0.73x, 직렬보다 느림), 토큰은 단일 에이전트 대비 3\~10배.
- **Opus 5 워커 3명**(2026-07-25, task-168): 실효 동시성 **1.36x**(팀 구간 19.2분 / 워커 wall 합 26.2분). 워커 토큰 합 35.8M(캐시 read 포함)·캐시 read 제외 2.3M, 같은 구간 리드는 6.1M·0.17M. 단일 에이전트 대비 배수는 대조군을 안 돌려 재지 못했다.

세대가 올라가도 **cold-start 는 줄지 않았다** — 프리픽스 재구성은 모델 능력이 아니라 구조적 비용이라, 워커별 캐시 write 가 0.18\~1.6M 인 동안 리드는 88k 였다. 그리고 실효 동시성을 실제로 좌우한 것은 모델이 아니라 **태스크 크기 균질성**이었다: 19.2분짜리 하나에 3.7·3.3분짜리 둘이라, 짧은 둘이 끝난 뒤 구간의 3/4 을 한 명이 혼자 돌았다. 따라서:
