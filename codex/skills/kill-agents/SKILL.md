...
Claude Code의 tmux teammate **잔재를 정리**한다. Codex agent나 worktree를 정리하는 스킬이 아니다. 부모 세션을
...
`컨텍스트 리셋` 는 **대화만** 비운다. 아래 둘은 별개 레이어라 따로 정리해야 한다:
...
> ⚠ 셸 alias `killagents` 는 **비대화형 쉘 명령 툴 PATH 에 없다**(`command not found`).
...
스크립트 stdout 을 요약한다(종료한 프로세스/pane, 로스터에서 제거한 멤버, 없으면 "정리할 잔재 없음"). TUI 로스터가 즉시 갱신 안 되면 한 번 더 `컨텍스트 리셋` 하거나 세션을 재시작하면 반영된다고 안내한다.
