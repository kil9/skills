...
- Invoke this skill explicitly with `$impeccable`. Do not activate it implicitly for ordinary frontend work.
- Use only Codex's built-in `image_gen` tool through the installed `imagegen` skill. Never use `scripts/generate-image.mjs`, `OPENAI_API_KEY`, or another metered image API as a fallback.
- If built-in image generation is unavailable or its included Codex quota is exhausted, stop image generation and report it instead of switching billing paths.
- Impeccable hooks are not installed in this setup. Do not enable or install them unless the user explicitly changes this policy.
- Anonymous choice telemetry is disabled in the bundled script.
- Version 4.0.4 is pinned in the managed skills repository. Never run `npx impeccable install` or `npx impeccable update`; upgrades require a fresh audit and an explicit repository change.
...
1. Run `node <skill-base-dir>/scripts/context.mjs` once per session, replacing `<skill-base-dir>` with this loaded skill's directory and keeping cwd at the user's project. Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
...
**Pin / Unpin:** `node <skill-base-dir>/scripts/pin.mjs <pin|unpin> <command>` creates or removes a standalone `$<command>` shortcut. Replace `<skill-base-dir>` with this loaded skill's directory. Report the script's result concisely; relay stderr verbatim on error.

**Hooks:** Disabled by local policy. Do not run hook installation or management commands unless the user explicitly asks to change that policy in a later turn.
