...
전체 코드베이스(`$ARGUMENTS` 가 디렉토리/파일 경로면 그 범위로 제한)에 잠재된 버그를 찾아 `BUG_REPORT.html` 로 정리한다. 아래 단계들을 `update_plan` 으로 등록해 진행 상태를 갱신한다.

## 1. 코드베이스 파악

`README.md`/`AGENTS.md`/`CLAUDE.md` 가 있으면 먼저 읽어 도메인·아키텍처를 파악한다. **직접 읽기가 기본이다.** 핵심 코드 약 5,000 라인 이하면 핵심 파일 전부를 `sed`/`rg` 등 적절한 읽기 도구로 정독한다. 그 이상이어도 먼저 진입점·인증·라우팅·핵심 도메인 모듈을 직접 정독하고, **그러고도 컨텍스트에 안 들어가는 나머지에 한해** worker agent 탐색 요약으로 대체한다 — 요약은 원문보다 놓치는 것이 많으니 넘칠 때의 폴백이지 기본 경로가 아니다. 약 10,000 라인을 넘으면 사용자에게 알리고 핵심 영역부터 진행한다. 정독 중 의심스러운 패턴은 메모해 뒀다가 서브에이전트 프롬프트의 "특별히 봐달라" 항목으로 전달한다.
...
필요하면 먼저 `tool_search` 로 multi-agent 도구를 노출한다. **한 메시지에 4개 Codex worker agent 를 동시 호출**한다 (병렬 실행).
...
각 프롬프트에 포함할 것: "PR diff 리뷰가 아니라 코드베이스 전체 리뷰다. 모든 파일을 새 PR 이라 가정하고 검토하라" + 저장소 경로, 서비스 목적(1\~2문장), 운영 환경(nginx 뒤/K8s 등), 검토 대상 파일 목록, "특별히 봐달라" 항목(5\~15개), 발견 형식(파일:라인 / severity(Critical\~Info) / 트리거 시나리오 / 권장 수정안 / 짧은 코드 인용), "코드 근거(파일:라인)를 짚을 수 있으면 확신이 없어도 보고하라 — 뒤의 재검증 단계가 거른다. 근거 없는 추측은 빼라".

**거절되면 분류기를 의심한다.** cyber/bio 안전 분류기가 정상적인 보안 리뷰를 오탐으로 거절할 수 있다. 프롬프트를 바꿔 가며 우회하지 말고 무엇이 거절됐는지 사용자에게 그대로 보고한다 — 거절을 "이슈 없음" 으로 넘기면 거짓 음성이 된다. 같은 규칙이 `scan-bugs`·`scopic-deep-review`·`resolve-security-alerts` 세 곳에 있다 — repo 가 갈려 공유 파일을 못 쓰니 한쪽을 고치면 나머지도 함께 고친다.
...
`apply_patch` 로 저장소 루트(범위가 디렉토리면 그 안)에 생성한다. **재검증된 severity 기준**으로 정리하되 1차 발견과의 변화를 보여준다. 한국어로 작성.
