...
병렬은 [`../references/parallel-worktree.md`](../references/parallel-worktree.md) 의 병렬 적합성 게이트(워커 cold-start·낮은 실효 동시성·토큰 배수의 함정)를 **인지하고도** 이득이 분명할 때만 쓴다: ready set 에 substantial 독립 태스크(다파일·신규·리팩터급이고 서로 파일이 안 겹침) ≥ 2개일 때. 애매하면 솔로. 선택 사유를 한 줄 보고한다.
