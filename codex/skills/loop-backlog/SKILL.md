...
매 라운드 `bash ~/.codex/skills/references/backlog-context.sh`를 **한 번만** 호출해 snapshot만 읽고 목록·상세를 따로 재조회하지 않는다. exit 2면 backlog 가 없는 것이니 `$init-backlog`(옛 `PLAN.md` 만 있으면 `$migrate-to-backlog`)을 안내하고 중단; exit 3은 CLI 설치; 다른 non-zero는 오류 보고 후 라운드 중단이다. 공통 전제는 [`../references/backlog-basics.md`](../references/backlog-basics.md)를 따른다.
