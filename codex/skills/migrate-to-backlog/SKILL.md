...
파일별 T/I/M 번호를 한 번에 받는다. 파일 탐색·개별 읽기를 따로 하지 않는다.

```bash
bash ~/.codex/skills/migrate-to-backlog/plan-context.sh --dump
```
...
`--integration-mode none` 으로 에이전트 지침 자동 주입을 막는다. 이어서 `backlog/config.yml` 을 표준으로
맞춘다.

공유 스크립트로 일괄 적용한다(멱등). 항목을 손으로 확인하지 않는다.

```bash
bash ~/.codex/skills/init-backlog/backlog-config-standard.sh
```

statuses·default_status·auto_commit 을 표준으로 맞추고, origin 리모트가 없을 때만 remoteOperations 를 끈다. 근거는 스크립트 헤더 주석 참조.
...
1. **AC 확정.** 원 항목 본문에 완료 조건이 드러나면 그대로 `--ac` 로 옮긴다. 여러 T-N 을 훑어 **완료
   조건이 불명확한 항목만 모아** `request_user_input` 으로 AC 를 확정한다(항목당 추측으로 채우지 않는다).
...
구현은 시작하지 않는다.** 이관된 태스크의 소비는 `$start-backlog`(태스크 하나)·`$loop-backlog`(자율 드레인) 몫이고,
