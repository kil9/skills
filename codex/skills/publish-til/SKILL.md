---
name: publish-til
description: (사외용) 준비된 정적 콘텐츠를 공개 GitHub `kil9/til` 에 새 페이지로 추가·push 해 GitHub Pages 로 배포한다. "TIL 올려줘 / 이거 공개 페이지로" 라고 할 때. 사내는 /publish-naver 다.
---

호출: `$publish-til [<slug>]`

인자가 kebab-case 슬러그면 새 페이지명으로 쓴다. 인자가 없거나 슬러그 형식이 아니면(주제 설명문 등) §1 대로 스스로 정하고, 설명문은 글감으로도 쓴다. 페이지는 날짜 아티클이면 `<현재연도>/<slug>/`, 리브 소개·아카이브 같은 비-날짜 상설 페이지면 `p/<slug>/` 에 놓인다(TASK-40 재편, t/ 래퍼 제거). 이 스킬의 기본 대상은 날짜 아티클이다.
...
```
bash ~/.codex/skills/publish-til/til-preflight.sh [<slug>]
```
...
- 인자가 kebab-case(소문자·숫자·하이픈) 슬러그면 그대로 쓴다.
- 인자가 없거나 슬러그 형식이 아니면 콘텐츠 주제를 파악해 슬러그를 스스로 정한다. 주제 설명문은
  글감으로도 쓰고, `<연도>/`가 시간을 분리하므로 연도 접두사는 붙이지 않는다.
- 정한 슬러그를 `til-preflight.sh <slug>`로 검사한다. exit 3(이미 존재)일 때만 다른 슬러그나
  덮어쓰기 여부를 묻는다. 덮어쓰기는 명시적으로 원할 때만 진행한다.
- 정한 슬러그는 §7 최종 보고에서 알린다.

---

## 2. 콘텐츠 소스 확보

`request_user_input` 으로 어떤 콘텐츠를 publish 할지 묻는다. 옵션은 다음 형태로 제시하되, Other 로 자유 입력을 허용한다.

- 단일 HTML 파일을 `index.html` 로 배치 (예: `/tmp/draft.html`)
- claude.ai artifact URL — 웹 페치 도구로 가져와 저장소 런북대로 프레임 런타임 제거·standalone 재조립 (`curl` 은 SPA 셸/403 이라 쓰지 않는다)
- 디렉터리 전체를 그대로 복사 (안에 `index.html` 필수)
- 빈 템플릿만 생성하고 사용자가 이후 직접 채움

### 2-1. 보안 사전 점검 (public — 사내용보다 엄격)

게시 전에 다음 패턴을 grep 으로 점검한다. 하나라도 발견되면 해당 라인을 보여 주고 `request_user_input` 으로 진행 여부를 묻는다. 사용자가 명시적으로 OK 하면 계속, 아니면 중단한다.
...
삽화를 넣기로 했으면 [`references/illustration.md`](references/illustration.md) 를 펴고 그 사양으로 생성한다. 생성된 PNG 를 페이지에 넣는 방법(리사이즈·WebP 압축·base64 임베드·마크업)은 저장소 `AGENTS.md` §2-1 을 따른다.
...
한 줄 제목은 스스로 정한다. 루트 갤러리의 길이·어투를 따르고 내용을 드러내는 제목을 고른다.
정한 제목은 §7에서 알린다.
...
`request_user_input` 없이 즉시 진행한다. 연결된 backlog 태스크가 있으면 상태·AC·notes를 같은 커밋에 포함한다(저장소 커밋 규칙).
