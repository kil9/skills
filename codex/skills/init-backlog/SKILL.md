...
**1. 인터뷰.** 조사로 메워지지 않는 디테일(문제 정의, 목표·비목표, 접근 방식과 근거, 작업 범위, 제약, 검증 전략, 우선순위, 태스크 의존 관계, 단독 실행 필요 여부)은 의문이 없어질 때까지 집요하게 `request_user_input` 으로 인터뷰한다. 추측으로 채우지 않고 줄글 자유 입력도 받지 않는다.
