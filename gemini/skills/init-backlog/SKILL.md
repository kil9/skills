...
**1. 인터뷰.** 조사로 메워지지 않는 디테일(문제 정의, 목표·비목표, 접근 방식과 근거, 작업 범위, 제약, 검증 전략, 우선순위, 태스크 의존 관계, 단독 실행 필요 여부)은 의문이 없어질 때까지 집요하게 `ask_question` 으로 인터뷰한다. 추측으로 채우지 않고 줄글 자유 입력도 받지 않는다.
...
**3. config 표준 적용.** 같은 스킬 루트의 `init-backlog/backlog-config-standard.sh` 로 일괄 적용한다(멱등 — 이미 표준이면 아무것도 바꾸지 않는다). 항목을 손으로 확인하지 않는다.
