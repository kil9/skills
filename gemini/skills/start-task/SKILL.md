...
- **태스크 번호 지정**(`7`, `task-7`, `T-7` 등, 여러 개 가능): 지정된 태스크만 지정 순서대로 진행한다. 이미 Done 이면 알리고 제외한다. 의존 미해소·Blocked 상태면 그 사실을 알리고 `ask_question` 으로 진행 방식(의존/막힘 먼저 해소, 그대로 강행, 제외)을 확인한다.
