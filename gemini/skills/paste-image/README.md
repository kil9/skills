...
원격 SSH 세션의 Claude Code에 **브라우저 Ctrl+V 한 번으로** 로컬 스크린샷/이미지를 넘겨준다.

## 이런 적 없으셨나요

원격 서버에 SSH로 붙어서 Claude Code로 작업 중이다.

- UI 버그를 보여주려고 **스크린샷을 찍었는데**, 클립보드는 로컬이라 원격 터미널에 붙여넣을 수가 없다.
- 매번 로컬에 저장 → `scp user@remote:/tmp/...` → Claude에게 경로 말해주기 — 4단계를 반복한다.
- 결국 귀찮아서 이미지 대신 텍스트로 대충 설명하고 넘어간다.

이 스킬 디렉토리는 두 가지 경로를 제공한다.

- **빠른 경로 (`pimg`, 권장)** — 상시 업로드 서버 + 고정 URL 북마크. LLM 왕복 없이 즉시.
- **스킬 경로 (`/paste-image`)** — Claude 가 일회용 서버를 띄우고 업로드를 기다렸다가 알아서 Read.

## 빠른 경로: `pimg` (상시 서버)

```
사용자: (챗창에서) ! pimg          ← 또는 아무 셸에서 pimg
 └─ 상시 HTTP 서버 보장 기동, 고정 토큰 URL 출력 (이미 떠 있으면 URL 만 재출력)
     └─ 사용자: 북마크해 둔 URL 열고 Ctrl+V
         └─ 업로드 완료 → 페이지가 원격 저장 경로를 로컬 클립보드에 자동 복사
             └─ 사용자: 챗창에 그 경로를 붙여넣기 → Claude 가 Read
```
...
```
사용자: /paste-image
 └─ Claude: 원격 서버에 임시 HTTP 서버 기동 (랜덤 토큰 URL)
     └─ 사용자: 안내된 URL을 로컬 브라우저에서 열고 Ctrl+V (또는 파일 드롭)
         └─ 업로드 완료 → 서버가 경로를 Claude에 전달하며 스스로 종료
             └─ Claude: 그 경로를 Read 해서 대화 컨텍스트에 이미지 로드
```

경로 복사/붙여넣기가 아예 필요 없는 대신 LLM 왕복(스킬 기동·대기·Read)이 끼어 빠른 경로보다 느리다. 원격 서버가 로컬 클립보드에 접근할 필요가 없다는 점은 두 경로 모두 같다 — **브라우저를 중간 다리**로 쓴다.

## 설치

이 스킬은 [kil9/skills](https://github.com/kil9/skills) 저장소의 `claude/skills/paste-image/` 에 포함돼 있다. 개인 dotfiles 부트스트랩이 `~/.claude/skills` 를 이 저장소의 `claude/skills` 로, `~/.local/bin/pimg` 를 `rc/bin/pimg` 로 심링크하므로 별도 설치 단계가 없다.
...
```
! pimg                # Claude Code 챗창에서 (셸 직접 실행도 동일)
pimg 9000             # 포트 지정
...
- Claude Code — `/paste-image` skill 을 쓰려면 필요. 서버 단독 실행에는 불필요.

## 파일 구조

```
paste-image/
├── SKILL.md              # /paste-image skill 본문 (LLM 지시: Bash 1회 + Read 1회)
